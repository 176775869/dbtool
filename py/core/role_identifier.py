...
DIRECTION_KW = {
    '新能源/锂电': ['锂', '稀土', '固态电池', '刀片电池', '钠电池', '麒麟电池', '电池', '钨', '钴', '镍', '盐湖', '能源金属', '磷化工'],
...
def load_reason_map(date_str):
    ...
    if not os.path.exists(_EXCEL_PATH):
        print(f'[WARN] 涨停原因表不存在: {_EXCEL_PATH}')
    ...
    try:
        ...
        print(f'[OK] 加载涨停原因: {sheet_name}，共 {len(reason_map)} 条')
        return reason_map
    
    except Exception as e:
        print(f'[ERROR] Excel读取失败: {e}')
        _REASON_CACHE[date_str] = {}
        return {}
...
def identify_roles(direction_name, data):
    ...
    # ---- 2. 筛选该方向的涨停股（优先用涨停原因表，兜底用板块字段） ----
    rel_limit = []
    for st in limit_stocks:
        code = st.get('code', '')
        reason = reason_map.get(code, '')
        if not reason:
            reason = st.get('sector', '')
        if any(kw in reason for kw in kw_list):
            rel_limit.append(st)
