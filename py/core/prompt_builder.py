...
def resolve_file(file_ref, base_dir, max_limit=0, max_qs=0):
    if file_ref.startswith('AUTO_LATEST:'):
        ...
        if 'limit' in os.path.basename(latest) or 'limit_up' in os.path.basename(latest):
...
def build_prompt(scene='replay', extra_note=None):
    ...
    for fr in sc['files']:
        content, res = resolve_file(fr, base_dir, max_limit=sc.get('max_limit_up', 0), max_qs=sc.get('max_qs_pool', 0))
